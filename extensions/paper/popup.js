...
function renderResult(data) {
    ...
    const chips = [];
    if (basic.year)
        chips.push(basic.year + '년');
    if (basic.venue)
        chips.push(esc(basic.venue));
    if (basic.citationCount != null)
        chips.push(`인용 ${basic.citationCount}회`);
    const linksArr = [];
    if (basic.doi)
        linksArr.push(`<a href="https://doi.org/${basic.doi}" target="_blank">DOI ↗</a>`);
    if (basic.arxivId)
        linksArr.push(`<a href="https://arxiv.org/abs/${basic.arxivId}" target="_blank">arXiv ↗</a>`);
    ...
    if (analysis) {
        ...
        if (kwHtml)
            parts.push(`<div class="kw-row">${kwHtml}</div>`);
    ...
    if (authors?.length) {
        ...
        parts.push(authors.map(a => {
            const metaParts = [];
            if (a.hIndex != null)
                metaParts.push(`h-index ${a.hIndex}`);
            if (a.citationCount != null)
                metaParts.push(`인용 ${a.citationCount.toLocaleString()}회`);
